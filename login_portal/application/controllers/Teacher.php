...
public function newPassChng(){
	...
	// check old password
	$old = $this->db->select("pass")->from("emp_login")->where("empid",$tid)->limit(1)->get()->row();
	...
	if($old->pass == $opass){
		...
		$up = $this->db->where("empid",$tid)->update("emp_login",$data);
		...
		if($up){
			...
			redirect('teacher/changePass','location');
		}
	}else{
		redirect('teacher/changePass?error=1','refresh');
	}
}

public function deleteRequest(){
	$id = $this->uri->segment(3);
	$del = $this->db->where("reqid",$id)->delete("emp_reqlev");
...
public function editRequest(){
	$this->load->view('header');
	$this->load->view('leftbar');
	$this->load->view('teacher/editLeaveRequest');
	$this->load->view('footer');
}

public function editValueSave(){
	if(isset($_POST['submit'])){	 
		 extract($_POST);
		...
		$this->db->where("reqid",$reqid)->update('emp_reqlev',$data);
...
public function printAttendance(){
	$this->load->view("teacher/printAttend");
}
