...
$(document).ready(function(){
 $('#formid').submit(function() {
  ...
  $.post(
            "index.php/account_edit/balance_transfer_insert",
            $("#formid").serialize(),
            function(data){
              if(data==1)
			  {
				 $('#hidemessage').fadeIn('slow').delay(2000).fadeOut('slow');
					setTimeout(function(){					
					window.location="index.php/account_edit/balance_transper";
					},2000)
			  }			  
			  else{
				  alert(data);
				  ...
				  <div class="col-md-11" style="min-height:50px;" id="hidemessage">
					...
					<div class="table-responsive">
					  ...
					  <ul class="nav nav-tabs" id="myTab">
						<li class="active"><a data-toggle="tab" href="#home">Balance Transfer</a></li>
			...
			<br/>
					 <form class="form-horizontal" role="form" action="" method="post" id="formid">
						<div class="form-group">
						  <label class="control-label col-sm-2" >Transfer Account:</label>
						  <div class="col-sm-6" >          
						...
						<div class="form-group">
						  ...
						  <div class="col-sm-6" >          
							<select class="form-control" name="recaccount" required>
									<option value="">--Select Account Name--</option>
									<?php 
										$sqlaccc=$this->db->select('*')->from('account_cre')->get()->result();										
										foreach($sqlaccc as $accidshowc){
									?>
										<option value="<?php echo $accidshowc->accountid?>"><?php echo $accidshowc->acc_name.'('. $accidshowc->accountid.')'?></option>
										<?php }?>
							</select>
						  </div>						  
						</div>
...
<!------------------------------------------ End of expense form ---------------->
 ...
 <!------------------------------------------------- Start list of view form ---------------->
					...
					<form class="form-horizontal" role="form" action="index.php/account_edit/search_balance_transfer" method="post">
						<div class="form-group">
						...
						<table id="example1" class="table table-bordered table-striped">
							...
							<tbody>
								...
								$nr=1; foreach($query as $row){
									$tranacc=$this->accmodone->accountinfo($row->tranAcc);									
									$recacc=$this->accmodone->accountinfo($row->recAcc);									
									?>
								<tr>
									<td><?php echo $nr++ ?></td>	
									<td><?php echo $row->invoice_id?></td>									
									<td><?php echo $tranacc->acc_name.'('.$row->tranAcc.')'?></td>
									<td><?php echo $recacc->acc_name.'('.$row->recAcc.')'?></td>
